...
def getFunnyTitle(input_string):
    client = Fireworks(api_key=FIREWORK_API_KEY)
...
# Set Section 3
st.write("### " + getFunnyTitle(sortedGenres[2]["genre"]))
st.write("More " + sortedGenres[2]["genre"] + " movies we think you'll Like")
movies_compile3 = [
    movie for movie in recommendedMovies
    if 'genres' in movie and sortedGenres[2]["genre"] in movie['genres']
]
#st.write(movies_compile3)
movieCompileImages3 = list()
for i, movie in enumerate(movies_compile3):
        if i<10:
            response = requests.get(movie['poster'], stream=True, timeout=10)
            if response.status_code == 200:
                movieCompileImages3.append(movie['poster'])
#st.write(images)
clickedMovie3 = clickable_images(
    movieCompileImages3,
    titles=[f"Image #{str(i)}" for i in range(5)],
    div_style={"display": "flex", "justify-content": "center", "flex-wrap": "wrap"},
    img_style={"margin": "5px", "height": "200px"},
...
st.write("Off the beaten path " + sortedGenres[-8]["genre"] + " movies watched by others with similar taste.")
movies_compile4 = [
    movie for movie in recommendedMovies
    if 'genres' in movie and sortedGenres[-8]["genre"] in movie['genres']
]
st.write(movies_compile4)
